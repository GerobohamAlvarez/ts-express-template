...
describe("Encrypt security module: ", () => {
    ...
    it("Should fail if secret do not meet requirements." , () => {
        ...
        expect(() => new EncryptService("a")).toThrow("Invalid key: The secret does not meet minimum requirements.");

    });

    it.each( texts )("Should encrypt the text '%s' and be able to decode it" , async ( text: string) => {

      const encryptor: IEncrypt = new EncryptService("randoms-256-bits-string-32-chars");
